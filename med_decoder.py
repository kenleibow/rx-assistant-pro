...
if not st.session_state.logged_in:
    ...
    # --- THE FIX: This st.stop() ONLY triggers if not logged in ---
    st.stop()
...
with st.sidebar:
    ...
    if total_inches > 0:
        ...
        elif 18.5 <= bmi < 25: st.success(f"BMI: {bmi} (Normal)"); bmi_category = "Normal"
        elif 25 <= bmi < 30: st.warning(f"BMI: {bmi} (Overweight)"); bmi_category = "Overweight"
        elif 30 <= bmi < 35: st.error(f"BMI: {bmi} (Obese)"); bmi_category = "Obese"
        elif 35 <= bmi < 40: st.error(f"BMI: {bmi} (Severe Obesity)"); bmi_category = "Severe Obesity"
        else: st.error(f"BMI: {bmi} (Morbidly Obese)"); bmi_category = "Morbidly Obese"
    st.markdown("---")
...
def analyze_single_med(indication_text, brand_name):
    ...
    elif "plavix" in name or "clopidogrel" in name: return {"risk": "Heart Disease / Stroke", "style": "risk-high", "questions": ["History of TIA/Stroke?", "Stent placement?", "Preventative or active treatment?"], "rating": "Standard to Decline."}
    elif "abilify" in name or "aripiprazole" in name: return {"risk": "Bipolar / Depression / Schizophrenia", "style": "risk-high", "questions": ["Hospitalizations in last 5 years?", "Suicide attempts?", "Ability to work?"], "rating": "Complex. Mild = Standard. Bipolar = Table Rating."}
    elif "entresto" in name: return {"risk": "Heart Failure (CHF)", "style": "risk-high", "questions": ["What is your Ejection Fraction?", "Any hospitalizations recently?"], "rating": "Likely Decline or High Table Rating."}
...
with tab1:
    ...
    with col_b: st.button("🔄 Clear", on_click=clear_single, key="clear_single_tab")
...
with tab2:
    ...
    with col_y: st.button("🔄 Clear List", on_click=clear_multi, key="clear_multi_tab")
    ...
    if st.button("Analyze Combinations", key="analyze_btn"):
        if multi_input:
            ...
            if valid_meds:
                ...
                st.download_button("📄 Download Combo Report", data=pdf_bytes, file_name="combo_report.pdf", mime="application/pdf", key="pdf_multi_btn")
...
with tab3:
    ...
    with col_i1:
        ...
        conditions = st.multiselect("Select Conditions:", sorted_conditions, key="condition_select")
    with col_i2:
    ...
    if conditions or is_smoker or bmi > 30:
        ...
        if conditions:
            ...
            st.download_button("📄 Download Impairment Report", data=imp_pdf, file_name="condition_report.pdf", mime="application/pdf", key="pdf_imp_btn")

st.markdown("---")
with st.expander("⚠️ Legal Disclaimer & Liability Information"):
    st.markdown("**1. Educational Use Only:** This tool is for informational purposes only... **4. Liability Waiver:** InsurTech Express accepts no liability for errors.")
